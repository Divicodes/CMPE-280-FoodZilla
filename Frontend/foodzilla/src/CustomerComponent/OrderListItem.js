...
export class OrderListItem extends Component {
  ...
  handleCancelOrder = () => {
    ...
    axios
      .put(`${baseUrl}/orders/edit/${orderId}`, { status: "cancelled" })
      .then((res) => {
        console.log(res);
  ...
  render() {
    ...
    delivery_address =
      ...
      " " +
      delivery_address[0]?.city +
      " " +
      delivery_address[0]?.zipcode;
    ...
    return (
      <div>
        <ListItem alignItems="flex-start">
            ...
            secondary={
              <React.Fragment>
                ...
                {order.ORD_TYPE === "delivery" ? (
                  <Typography>Delivery Address:{delivery_address}</Typography>
                ) : (
                  ""
                )}
                <Typography>
          ...
          <button
            className="btn btn-info"
            onClick={() => this.handleOpenReciept()}
          >
            View Reciept
          </button>
          <Dialog
